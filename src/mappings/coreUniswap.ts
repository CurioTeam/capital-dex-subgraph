...
import {
  ...
  ZERO_BD,
  BI_18,
  createLiquiditySnapshot, UNISWAP_FACTORY_ADDRESS
} from './helpers'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = Factory.load(UNISWAP_FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  token0.derivedETH = findEthPerToken(token0 as Token, UNISWAP_FACTORY_ADDRESS)
  token1.derivedETH = findEthPerToken(token1 as Token, UNISWAP_FACTORY_ADDRESS)
  token0.save()
  token1.save()
...
export function handleSwap(event: Swap): void {
  ...
  let capitalDexDayData = updateCapitalDexDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  capitalDexDayData.dailyVolumeUSD = capitalDexDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  capitalDexDayData.dailyVolumeETH = capitalDexDayData.dailyVolumeETH.plus(trackedAmountETH)
  capitalDexDayData.dailyVolumeUntracked = capitalDexDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  capitalDexDayData.save()
